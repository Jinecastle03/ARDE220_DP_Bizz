document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderSavedTokens() {
        ...
        const BASE_SPACING = 170;          // 기본 간격
        const BONUS_START_IDX = 7;         // 8번째부터 간격 늘리기
        const SPREAD_SCALE = 0.35;         // 8번 이후 간격 가중치
        const MIN_PATH_RATIO = 0.04;       // 시작 여유
        const WIGGLE_BASE = 14;            // 끝부분 경치 위/아래 흐름
        const WIGGLE_STEP = 6;             // 토큰이 늘어나면 마다 증산
        ...
        savedEmotionsData.forEach((emotion, idx) => {
            const spreadFactor = 1 + Math.max(0, idx - BONUS_START_IDX) * SPREAD_SCALE;
            const dist = Math.min(usableLen * 0.9, BASE_SPACING * idx * spreadFactor);
            const pointLen = Math.max(minLen, visibleEnd - dist);
            const endPt = dropPathSvg.getPointAtLength(pointLen);

            let offsetX = endPt.x;
            let offsetY = endPt.y;
            if (idx >= BONUS_START_IDX) {
                const delta = 2;
                ...
                const nx = -dy / mag;
                const ny = dx / mag;
                const wiggle = WIGGLE_BASE + (idx - BONUS_START_IDX) * WIGGLE_STEP;
                const dir = idx % 2 === 0 ? 1 : -1; // 위아래 번갈아 배치
                offsetX = Math.min(Math.max(endPt.x + nx * wiggle * dir, MARGIN_PX_X), viewBoxWidth - MARGIN_PX_X);
                offsetY = Math.min(Math.max(endPt.y + ny * wiggle * dir, MARGIN_PX_Y), viewBoxHeight - MARGIN_PX_Y);
            }
            
            const percentX = Math.min(Math.max((offsetX / viewBoxWidth) * 100, 3), 97);
            const percentY = Math.min(Math.max((offsetY / viewBoxHeight) * 100, 3), 97);
